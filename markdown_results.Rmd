...
Our control-only model (all years; only control individuals; n = `r nofood[, .N]`) found that hare home ranges decreased with hare density (`r round(NFdcoef, 2)` +/- `r round(NFdse, 2)` ha per hare/ha increase). As hare density increased from 0 to 1.2 hares/ha, hare home ranges shrunk from `r effs_NF[x == min(x), round(predicted, 2)]` ha to `r effs_NF[x == "1.2", round(predicted, 2)]`.
...
Our food treatment model (years with food-add experiments; all individuals; n = `r yesfood[, .N]`), found that as hare density increased from 0 to 1.2 hares/ha, controls slightly decreased their home ranges from `r effs_WF[group == "Control" & x == min(x), round(predicted, 2)]` ha to `r effs_WF[group == "Control" & x == "1.2", round(predicted, 2)]` ha, and food-adds substantially decreased their home ranges from `r effs_WF[group == "Food add" & x == min(x), round(predicted, 2)]` ha to `r effs_WF[group == "Food add" & x == "1.2", round(predicted, 2)]` ha (Figure 3). All terms in the model, hare density, food treatment, and their interaction, were significant (t \> 2) in predicting home range size (Table 1).
